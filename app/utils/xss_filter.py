...
Modified by yiyuhao:
1、取消掉对embed属性allownetworking的强制设置为none
2、定义一个函数接口html_clean()
3、object、embed增加部分可选择属性
4、图片设置max-width为100%
"""
import re
...
class XssHtmlParser(HTMLParser):
    allow_tags = ['a', 'img', 'br', 'strong', 'b', 'code', 'pre',
                  ...
                  'caption', 'small', 'q', 'sup', 'sub', 'object', 'param']
    common_attrs = ["style", "class", "name", "dir", "width", "height", "title"]
    nonend_tags = ["img", "hr", "br", "param"]
    tags_own_attrs = {
        "img": ["src", "alt", "align"],
        "a": ["href", "target", "rel"],
        "embed": ["src", "type", "allowfullscreen", "loop", "play", "wmode", "menu", "pluginspage", "quality"],
    ...
    def node_object(self, attrs):
        attrs = self._common_attr(attrs)
        attrs = self._get_link(attrs, "codebase")
        attrs["style"] = "max-width:100%"
        # tags中过滤掉了width
        # attrs["max-width"] = "100%"
        return attrs
...
def html_clean(markup):
    try:
        parser = XssHtmlParser()
        parser.feed(markup)
        parser.close()
        return parser.clean_html
    except:
